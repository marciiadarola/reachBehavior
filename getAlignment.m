...
% distractor_duration=250; % in ms
% distractor_freq=1/(distractor_duration/1000);
...
% Fnorm=(0.5*distractor_freq)/(moviefps/2);
% df=designfilt('highpassfir','FilterOrder',30,'CutoffFrequency',Fnorm);
% D=mean(grpdelay(df));
% temp_LED=filter(df,[temp_LED zeros(1,D)]);
% temp_LED=temp_LED(D+1:end);
...
%    plot(movie_LED_ITIs,'Color','b');
    ...
    if isInSecondHalf==true
        backup_arduino_LED_ITIs=arduino_LED_ITIs;
        midLength=floor(3*length(arduino_LED_ITIs)/4)+20;
...
%    tryinc=0.000025;
    ...
    if D>0
        ...
        trydelays=guess_best_delay-200:guess_best_delay+200;
